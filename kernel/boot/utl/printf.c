...
static const char* itoa(uint32_t val,char* buffer,size_t len,size_t* plen,uint32_t base);
static int ks_vprintf(struct out_t* out,const char* format,va_list ap); 
...
int vprintf(const char* format,va_list ap) {
	...
	auto size_t vprintf_room(size_t size) {
		return size;
	}

	auto void vprintf_output(int c) {
		ks_putchar(c);
	}

	struct out_t out = { 
		.room = vprintf_room,
		.output = vprintf_output,
	};

	return ks_vprintf(&out,format,ap);
}
...
static int ks_vprintf(struct out_t* out,const char* format,va_list ap) {
	...
	LABEL(fmt_d)
		...
		if (fmt.size == 16) {
			val.u16 = va_arg(ap,uint32_t);
			if (val.i16 < 0) {
				fmt.sign = 1;
				val.i16 = - val.i16;
			}
		} else if (fmt.size == 8) {
			val.u8 = va_arg(ap,uint32_t);
			if (val.i8 < 0) {
				fmt.sign = 1;
				val.i8 = - val.i8;
			}
		...
		v = itoa(val.u32,tmp,sizeof(tmp),&len,10);
		if (v == 0) goto failure;
	...
	LABEL(fmt_u)
		...
		if (fmt.size == 16) {
			val.u16 = va_arg(ap,uint32_t) & 0xFFFF;
		} else if (fmt.size == 8) {
			val.u8 = va_arg(ap,uint32_t) & 0xFF;
		} else {
			val.u32 = va_arg(ap,uint32_t);
		}
		...
		v = itoa(val.u32,tmp,sizeof(tmp),&len,fmt.base);
		if (v == 0) goto failure;
